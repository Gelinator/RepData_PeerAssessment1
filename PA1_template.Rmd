...
plot(total$date,total$sum, type="h",xlab="Period", ylab="Total number of steps")
````
...
plot(inter$interval,inter$AverageSteps, type="l",xlab="Interval", ylab="Average number of steps")
````
...
## The strategy adopted was to replace the missing values by the median for their respective time interval.
...
plot(cleaningUp$date,cleaningUp$sum, type="h", xlab="Period", ylab="Total number of steps")
```
...
qplot(interval,MeanSteps, data = IntervalDay, geom="line",ylab="Average Number of Steps",xlab = "Time Intervals") + facet_wrap(~daytype,nrow=2)
```
